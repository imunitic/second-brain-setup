...
`/sb-design-note`) into a single tracked task, using the vault's existing task-tracking
machinery instead of a bespoke format: creation goes through `/sb-note --task`, and status
transitions from then on belong entirely to the `sb-task` skill. This command's only job is
...
already works (see `sb-task`'s "Task file structure").

## Usage

```
/sb-task-note "topic"    # Compile the task note for a Ready design note
```

No `--continue`/`--list` here — once created, the task note's own progress (checked items,
`status:` frontmatter) is what `/sb-note --list` and the `sb-task` skill already track.
...
- No matching note → "No Ready design note found for '{topic}'. Run
  `/sb-design-note \"{topic}\"` first." Never generate a checklist from scratch.
...
- Matching note but `Status: Reference` → "Design note for '{topic}' concluded as Reference —
  nothing to compile. Reopen it with `/sb-design-note \"{topic}\"` and mark it Ready if that's
  changed."
...
3. For each step, write it the way `sb-task`'s own checklist convention expects: a short
...
Follow `/sb-note`'s task-mode procedure exactly (its "Creating the note" section) — don't
...
- **Title:** a short, plain description of the compiled plan (e.g. "Rollup direct storage
  implementation") — `/sb-note --task` resolves the project prefix and `task_id` and prepends
  them itself.
- **Project:** derive from the source design note's `project:` frontmatter — that prefix is already
  resolved (the design note went through `/sb-design-note`'s resolution when it was created,
...
- **`## Notes` (pre-implementation):** populate per `sb-task`'s own convention —
...
IN-PROGRESS → REVIEW) happen automatically via the `sb-task` skill once work starts — this
command doesn't set or watch that field itself.

## Integration

- Reads `/sb-design-note`'s `Ready` output only — never invents a design.
- Delegates note creation to `/sb-note --task` and status tracking to the `sb-task` skill
...
```
User: /sb-task-note "rollup direct storage"
...
Claude: [reads designs/{PROJECT} — Rollup direct storage.md, Status: Ready]
        ...
        Status transitions happen automatically via the sb-task skill once you start work.
```
