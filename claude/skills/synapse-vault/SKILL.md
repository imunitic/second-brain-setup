...
them. `_index.json`, `_refs.tsv` and `_tags_cache.json` are machine-only. `_profile.txt` and
`_manifest.tsv` are human-readable and may be edited directly.

Fuller write-ups, when the summary above is not enough:
`research/Second-brain tooling gotchas — command-owned folders and vault_patch heading targets`.
