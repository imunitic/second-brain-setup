...
| Who depends on a subsystem, or what it depends on | `synapse-query.sh links "{Node}" --inbound` / `--closure` | Real transitive-closure traversal over the typed relations, node-granularity (not per-symbol — there's no exact call graph yet). |
